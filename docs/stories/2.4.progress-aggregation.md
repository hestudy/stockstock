...
- `pnpm vitest run apps/web/src/app/api/v1/optimizations/__tests__`
- `pnpm vitest run "apps/web/src/app/(dashboard)/optimizations/[id]/__tests__/page.test.tsx"`

### Completion Notes List

- 实现作业详情页进度条、停机提示与 Top-N 排序提示，并增加导出与复运行交互，前端改动集中在 `page.tsx` 与新增的 Vitest 组件测试。
- 扩展 shared 类型与 orchestrator（TS/Python）以携带 `resultSummaryId`、`sourceJobId`，新增导出与复运行 API 路由及对应集成/单元测试。
- 后端与前端服务层加入导出/复运行方法，API 测试覆盖导出 200/4xx、复运行鉴权，Python Pytest 校验聚合终值与导出打包逻辑。
- Playwright 场景与历史页面刷新尚未落地，需在后续 Story/任务中补充。
