...
- 新增 Python Worker 消费流程，识别错误类型、记录队列等待/活跃作业指标并配套单测。
- 为内存编排存储引入可重入锁，保障并发安全并补充线程化验证。
- 同步数据库脚本与架构文档，补充 `optimization_tasks` 的 `next_run_at`、`throttled`、`last_error` 字段及索引。
