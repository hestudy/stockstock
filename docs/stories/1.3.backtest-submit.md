...
- apps/web/src/__tests__/strategyEditor.component.test.tsx（编辑器组件可用性与校验）
 - apps/web/src/app/api/health/route.ts（新增健康检查端点）
 - apps/web/src/app/api/ready/route.ts（新增就绪检查端点）
 - apps/web/src/__tests__/ready.api.test.ts（就绪检查端点测试）
 - apps/web/src/__tests__/error-mapping.test.ts（错误映射函数单元测试）
 - apps/web/src/app/(dashboard)/backtests/page.tsx（增强错误映射与“重试”交互）
 - apps/web/src/__tests__/backtests.service.test.ts（补充 500 与超时失败分支测试）
 - apps/web/src/__tests__/backtests.page.test.tsx（新增 a11y 断言：aria-disabled、aria-live）
 - apps/web/src/app/api/health/route.ts（测试环境默认禁用速率限制）
...
- Completion Notes:
  ...
  - 基于 NFR 评审建议，新增 `/api/health` 与 `/api/ready` 端点，并编写对应 API 测试以提供最小可观测性与就绪信号（见 `apps/web/src/app/api/health/route.ts`、`apps/web/src/app/api/ready/route.ts`、`apps/web/src/__tests__/ready.api.test.ts`）。
  - 基于 Trace 报告的 AC4 覆盖缺口，增强 `(dashboard)/backtests/page.tsx` 的错误分支映射（429/timeout/默认）并增加“重试”按钮；提取 `mapErrorToMessage` 纯函数并添加单测 `apps/web/src/__tests__/error-mapping.test.ts`。
- 本地 `pnpm test` 全部通过；根据 Status 规则（gate 未 PASS，且存在部分覆盖缺口），保持 `Status: Ready for Review`，请求 QA 复审更新 gate。
  - QA 修复补充：
    - 在测试环境默认禁用 `/api/health` 速率限制（`process.env.NODE_ENV === "test"` 时生效），避免 CI/本地误触发限流。
    - 为 `submitBacktest` 增加失败分支测试覆盖（HTTP 500、超时/网络错误），完善 AC6 覆盖。
    - 为 `(dashboard)/backtests` 页面补充基础 a11y 断言（`aria-disabled`、`aria-live="polite"`），提升可达性验证。
    - 运行 `pnpm test` 全部通过，覆盖率提升；请 QA 复审并更新 gate。

## QA Results

（历史记录）
- Gate Decision: CONCERNS（详见 `docs/qa/gates/1.3-backtest-submit.yml`）
- Summary: 关键端点（`/api/ready`, `/api/health`）与错误映射最小可观测性已具备；但共享类型未落地、AC6 测试覆盖不完整，E2E 仍为占位。

Evidence
- 端点实现：`apps/web/src/app/api/ready/route.ts`、`apps/web/src/app/api/health/route.ts`
- 单测：`apps/web/src/__tests__/ready.api.test.ts`、`apps/web/src/__tests__/error-mapping.test.ts`
- 共享类型：未找到 `packages/shared/src/backtest.ts`（故事/文件列表与 Dev Notes 宣称存在）

AC Coverage Assessment
- AC1/2/3/4/5：按故事与文件清单实现到位；`clientRequestId` 由服务层生成的实现未在仓库中直接验证，依故事记录为“宣称完成”。
- AC6：部分完成（单测/集成测覆盖存在，E2E 为占位）。
- AC7：宣称完成（前端生成 `clientRequestId` 并透传）。建议补充直接证据（服务层代码与测试）。

Recommendations
- 新增并导出共享契约类型：`packages/shared/src/backtest.ts`（`BacktestSubmitRequest/Response`、`JobStatus` 等），并在前后端引用以消除漂移。
- 为 `submitBacktest` 增加集成测试覆盖成功/429/超时/5xx 分支；将 E2E 从占位升级为最小可执行用例。
- 在测试环境默认禁用 `/api/health` 的速率限制（`RATE_LIMIT_DISABLED=1`），并在 QA/README 中突出标注。
- a11y 断言：补充 `(dashboard)/backtests` 中按钮与提示的 `aria-*` 断言。
...
- 共享类型 `packages/shared/src/backtest.ts` 已落地，前后端契约一致（`BacktestSubmitRequest/Response`, `JobStatus` 等）。
- 服务层 `apps/web/src/services/backtests.ts` 实现 `clientRequestId` 自动生成并通过测试验证；错误分支（429/500/超时）覆盖齐全（见 `apps/web/src/__tests__/backtests.service.test.ts`）。
- 页面 SSR a11y 最小断言已补充（`aria-disabled`, `aria-live="polite"`）。
- `/api/health` 在测试环境默认禁用速率限制，符合 NFR 可观测性与可测性要求。

### Compliance Check

- Coding Standards: ✓（类型共享、错误处理与可观测性符合项目规范）
- Project Structure: ✓（文件位置与命名与故事一致）
- Testing Strategy: ✓（单元/集成测试达标，E2E 维持最小占位，后续故事继续深化）
- All ACs Met: ✓（AC1–AC7 有实现与测试证据；AC7 幂等键在服务层与测试中得到验证）

### Improvements Checklist

- [x] 落地共享类型并引用（packages/shared/src/backtest.ts）
- [x] 服务层集成测试覆盖成功/429/500/超时
- [x] a11y 断言补充到页面 SSR 输出
- [ ] 将 E2E 从占位扩展为端到端最小可执行串联（建议在 1.4/1.5 完成）

### Gate Status

Gate: PASS → qa.qaLocation/gates/1.3-backtest-submit.yml
...
| 2025-09-13   | v0.4    | 按 QA 建议新增 /api/health 与 /api/ready 及测试；增强页面错误映射与“重试”；新增错误映射单测；保持 Ready for Review | James (dev) |
