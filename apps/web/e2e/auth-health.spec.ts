...
test.describe("Auth & Health Canary (Story 1.1)", () => {
  ...
  test("Protected dashboard route redirects unauthenticated users to /login", async ({ page }) => {
    ...
    const friendlyCN = await page
      .getByText(/请先登录|没有访问|未登录/)
      .first()
      .isVisible()
      .catch(() => false);
    const friendlyEN = await page
      .getByText(/sign in|not authorized|please login/i)
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test("Authenticated (bypass) user can access /health and see HealthCard", async ({
    page,
    context,
  }) => {
