...
vi.mock("../../../../../_lib/auth", () => ({ getOwner: vi.fn() }));
vi.mock("../../../../../_lib/rateLimit", () => ({ rateLimit: vi.fn() }));
vi.mock("../../../../../_lib/validate", () => ({ isValidBacktestId: vi.fn() }));
...
function makeReq(url = "http://localhost/api/v1/backtests/abc/result"): Request {
...
describe("result route", () => {
  ...
  it("200 OK on happy path", async () => {
    ...
    expect(body).toMatchObject({ id: "job-1", metrics: expect.any(Object) });
    const payload = (spy.mock.calls.at(-1) as any)?.[1];
    expect(payload).toMatchObject({ kind: "http_server", route: "/api/v1/backtests/[id]/result", method: "GET", status: 200 });
  ...
  it("400 on invalid id", async () => {
    ...
    const res = await GET(makeReq("http://localhost/api/v1/backtests/bad/result"), ctx("bad"));
